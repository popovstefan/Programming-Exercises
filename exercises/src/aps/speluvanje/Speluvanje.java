...
class Zbor implements Comparable<Zbor> {
    ...
    @Override
    public int hashCode() {
        return Math.abs(zbor.hashCode() % 73);
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (String s : text) {
            ...
            if (!Character.isLetter(lastChar)) {
                String key = s.substring(0, s.length() - 1).toLowerCase();
                if (tabela.search(new Zbor(key)) == -1) {
                    System.out.println(key);
                    hasIncorrect = true;
                }
            } else {
                if (tabela.search(new Zbor(s.toLowerCase())) == -1) {

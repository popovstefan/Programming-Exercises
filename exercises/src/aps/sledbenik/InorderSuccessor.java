...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    public E getSuccessor(BNode<E> t, E x) {
        ...
        if (node == null)
            return null;
        BNode<E> successor = null;
        BNode<E> current = t;

        while (current != null && !current.info.equals(node.info)) {
            if (current.info.compareTo(node.info) > 0) {
                successor = current;
        ...
        if (current.right == null)
            return successor == null ? null : successor.info;
        current = current.right;
